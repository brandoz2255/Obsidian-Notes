- From the intro the first thing it tells us is that anl=alyzing the traffice is fucking hard so we will be loosing our minds when doing this in Blue team 
- One key takeaway is might got this since it willl show us all the tools that are needed to look at this shit here is our first command 

```shell-session
00818@htb[/htb]$ wget -O file.zip 'https://academy.hackthebox.com/storage/resources/pcap_files.zip' && mkdir tempdir && unzip file.zip -d tempdir && mkdir -p pcaps && mv tempdir/Intermediate_Network_Traffic_Analysis/* pcaps/ && rm -r tempdir file.zip
...
- installs what we need for our pawn boxes